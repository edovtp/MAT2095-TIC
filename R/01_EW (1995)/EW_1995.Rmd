...
here::i_am("01_EW (1995)/EW_1995.Rmd")
set.seed(2211)

## Extra functions
source('code/00_extras.R')
source('code/01_dirichlet_process.R')
source('code/02_dirichlet_process_mixtures.R')

# extra_functions_files <- list.files('code/')
# sapply(extra_functions_files, 
#        function(filename) source(here('R', 'code', filename),
#                                  .GlobalEnv)
# )
```
...
The algorithm is implemented in `01_Escobar (1995)/algorithm1.R`
...
We simulate a sample of size 100 from a Dirichlet Process Mixture Model with parameters $M = 1$, $m = 0$, $\tau = 100$, $s = 10$ and $S = 2$. The reason for this is:

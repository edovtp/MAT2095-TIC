...
Con esto, el paso de simular de $\theta|\mathbf{y}$ podemos dividirlo en dos: simular primero de $\mathbf{c}|\mathbf{y}$ y luego de $\theta^*|\mathbf{c}, \mathbf{y}$. Para la segunda distribución, denotando por $\mathcal{C}_j = \{i\colon c_i = j\}$, tenemos que:
...
- Para $t = 1, ... T$:
    ...
    1.   Simular $\phi \sim \text{Beta}(M + 1, n)$ y calcular $\pi = (\alpha + k - 1)/[n(\beta - \log(\phi)) + \alpha + k - 1]$
    2.   Muestrear de  $$M|\phi, k \sim \pi\text{Gamma}(\alpha + k - 1, \beta - \log(\phi)) + (1 - \pi)\text{Gamma}(\alpha + k - 1, \beta - \log(\phi))$$
    ...
    4. Para $i = 1, ..., n$:
        ...
        M\int f_\theta(y_i) dG_\eta(\theta) & j = k_{-i} + 1
        \end{cases}$$
        4.2 Reordenar los valores de $\mathbf{c}$

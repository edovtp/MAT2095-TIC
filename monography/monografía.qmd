...
\begin{ejemplo}[Proceso de Pitman-Yor]
  ...
  Para replicar ambas figuras, ver los códigos \texttt{01\_Py.jl} y \texttt{02\_PyData.jl}.
\end{ejemplo}
```
...
La generación de distribuciones discretas puede no ser adecuada para diferentes problemas, por lo que entonces, con el objetivo de formular modelos que realmente reflejen el problema considerado, nos interesa extender los procesos de Dirichlet [^4].
...
Luego, notamos que el uso de estas distribuciones entrega una extensión natural de los modelos de mezcla finita a un número infinito de componentes. En lo que sigue, nos concentraremos en el caso $G \sim \text{DP}(M, G_0)$, pero debemos recordar que en la práctica podemos utilizar cualquier medida de probabilidad aleatoria. Con esta elección, el modelo dado en ([-@eq-dpm-mezcla]) recibe el nombre de **Dirichlet Process Mixture Model (DPM)**.
...
Recordemos que nuestra motivación inicial para estudiar medidas aleatorias es la estimación de la densidad a posteriori predictiva $p(y_{n+1}|\mathbf{y})$, lo cual podemos obtener al integrar con respecto a la variable latente $\theta_{n+1}$ correspondiente, esto es,

$$
p(y_{n+1}|\mathbf{y}) = \int f_{\theta_{n+1}}(y_{n+1})dP(\theta_{n+1}|\mathbf{y})
$$

Lo anterior puede llegar a ser muy complejo de calcular analíticamente (esto no es algo que ocurra solo con modelos no paramétricos), por lo que tenemos que recurrir al uso de métodos de simulación. En particular, si denotamos por $\theta_{n+1}^{(t)}$, $t=1, ...T$, muestras de $\theta_{n+1}|\mathbf{y}$, entonces
...
```{=tex}
\begin{proposicion}
  Considerando el modelo dado en (7), se tiene que
  $$
  G | y \sim \int \text{DP}(M + \sum_{i=1}^n \delta_{\theta_i})dP(\theta|\mathbf{y})
  $$
\end{proposicion}
```

Ahora, si en el capítulo anterior vimos que era difícil simular de la posteriori de $G$, este caso es aún peor, por lo cual debemos considerar una segunda opción, que sí sea feasible, para obtener las muestras deseadas. Al igual que antes, la idea es trabajar con la marginalización con respecto a $G$, i.e. la representación por urnas de Pólya. En este caso, el proceso de simulación consiste en:

1. Simular de $p(\theta|\mathbf{y})$
2. Simular de $p(\theta_{n+1}|\theta)$

Notamos que el segundo paso ya sabemos cómo realizarlo, que es utilizando la distribución dada en ([-@eq-dp-marginal]) o, equivalentemente, la dada en ([-@eq-dp-marginal-equal]), con $i = n + 1$. Así, lo nuevo en este caso será la obtención de $p(\theta|\mathbf{y})$ y su correspondiente simulación.

## Simulación a posteriori

Una de las formas que tenemos de simular de la distribución deseada es utilizando el algoritmo de Gibbs [@geman_stochastic_1984], la cual requiere la obtención y posible simulación de las distribuciones condicionales completas, que en este caso corresponden a $p(\theta_{i}|\theta_{-i}, \mathbf{y})$.

La obtención de estas distribuciones 
...
De manera más general podemos incluir 

...
Con esto, el paso de simular de $\theta|\mathbf{y}$ podemos dividirlo en dos: simular primero de $\mathbf{c}|\mathbf{y}$ y luego de $\theta^*|\mathbf{c}, \mathbf{y}$. Para la segunda distribución tenemos que:

$$
\begin{aligned}
  p(\theta^*|\mathbf{c}, \mathbf{y}) &\propto p(\mathbf{y}|\theta)p(\theta^*|\mathbf{c}, \mathbf{y}) \\
  &= \left[\prod_{i=1}^k \prod_{\{j\colon c_j = i\}} f_{\theta_i^*}(y_j) \right] \prod_{i=1}^k G_\eta(\theta_i^*) \\
  &= \prod_{i=1}^k \left[G_\eta(\theta_i^*) \prod_{\{j\colon c_j = i\}} f_{\theta_i^*}(y_j)\right]
\end{aligned}
$$

Donde la segunda distribución en el producto se obtiene al notar de ([-@eq-dp-marginal]) que un nuevo valor único siempre viene de $G_\eta$. Luego, tenemos que dados $\mathbf{c}$ y $\mathbf{y}$, cada uno de los valores únicos son independientes, obteniendo finalmente que:

$$
p(\theta_i^*|\mathbf{c}, \mathbf{y}) \propto G_\eta(\theta_i^*)\prod_{\{j\colon c_j = i\}} f_{\theta_i^*}(y_j)
$$

Con esta información, podemos simular de la distribución deseada muestreando uno por uno $p(\theta_i^*|\mathbf{c}, \mathbf{y})$, $i = 1, ..., k$.

Para simular de $\mathbf{c}|\mathbf{y}$ utilizaremos nuevamente el algoritmo de Gibbs, por lo que necesitamos obtener las distribuciones condicionales completas $c_i|c_{-i}, \mathbf{y}$. Estas distribuciones se pueden obtener directamente de ([-@eq-dpm-simple]) al integrar con respecto a $\theta$. Denotando por $k_{-i}$ el número de valores únicos de $\theta_{-i}$ y por $n_{-i, j}$ el número de observaciones con $c_j = i$, $j\neq i$, obtenemos que:

$$
p(c_i = j| c_{-i}, \mathbf{y}) \propto \begin{cases}
  n_{-i, j} \int f_\theta(y_i) dH_{-i}(\theta) & j = 1, ..., k_{-i} \\
  M\int f_\theta(y_i) dG_\eta(\theta) & j = k_{-i} + 1
\end{cases}
$$

donde $H_{-i} \propto G_\eta(\theta) \prod_{\{l\colon c_l = j, l \neq i\}} f_{\theta}(y_l)$ y el caso $j = k_{-i} + 1$ es que el que corresponde a que la observación pertenezca a un cluster que previamente estaba vacío.

### Hiperparámetros aleatorios

Agreguemos ahora la inferencia tanto para $M$ como para $\eta$. Para esto, nos apoyaremos de algunas distribuciones conocidas.


Incluyamos ahora en el modelo conjugado con verosimilitud Normal la inferencia sobre $M$ y $\eta$, así como la separación de $\theta$. Tenemos que:
...
* Generalización del modelo con verosimilitud Normal
...
**NOTA**: apuntar al resumen del apéndice para el algoritmo de forma más compacta con toda la notación.

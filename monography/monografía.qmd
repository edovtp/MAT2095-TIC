...
Así, cuando hablamos de cuantificar la incertidumbre con probabilidades, podemos notar que siempre nos hemos estado refiriendo a incertezas ontológicas. En este sentido, las probabilidades se interpretan como la frecuencia de ocurrencia de un evento, considerando un número infinito de repeticiones, lo cual podemos justificarlo a través de la Ley Fuerte de los Grandes Números. Este es el paradigma *frecuentista* de la estadística.

Ahora, ¿por qué no podemos modelar también las incertezas epistemológicas mediante probabilidades?. Esto es precisamente, de manera justificada, lo que propone el paradigma *Bayesiano*. En este caso ya no podemos interpretar las probabilidades solo como frecuencias, si no que también como una *medida racional de incerteza*, lo cual normalmente dependerá de cada persona.
...
Consideremos, en primer lugar, el problema de estimación de densidades, lo cual en el contexto bayesiano se traduce a la estimación de la posteriori predictiva $p(y_{n+1}|\mathbf{y})$. Específicamente, consideremos una muestra $y_1, ..., y_n$ condicionalmente independiente dado $G$, donde $G$ es una medida de probabilidad no especificada, denotado por $y_1, ..., y_n | G \overset{i.i.d.}{\sim} G$.
...
Utilizando el Teorema de Extensión de Kolmogorov, Ferguson muestra que, bajo ciertas condiciones de regularidad, $G$ existe para todo $G_0$. Además, señala ciertas propiedades de los DP, algunas de las cuales se deducen directamente de la definición del proceso al considerar la partición $\{B, B^c\}$.
...
Considerando una muestra aleatoria $y_1, ..., y_n|G \sim G$, Blackwell y MacQueen [-@blackwell_ferguson_1973] formulan una representación de la distribución marginal $p(y_1, ..., y_n)$ mediante una construcción por Urnas de Pólya. En particular, sabemos que la distribución marginal conjunta podemos expresarla como

$$
p(y_1, ..., y_n) = p(y_1)\prod_{i=2}^n p(y_i|y_1, ..., y_{i-1})
$$

y lo que muestran ambos autores es que $p(y_1)$ tiene la misma distribución que $G_0$ y que, para $i = 2, ..., n$,
...
Ahora, la construcción anterior sigue teniendo un problema, ya que en la práctica no podemos repetir el proceso de obtención de pesos y átomos de manera infinita. Una de las formas de evitar este problema es simplemente truncar la representación hasta un número $H$ fijo, fijando $\upsilon_H = 1$, denominado un \textbf{DP finito} [@ishwaran_markov_2000], u obtener los pesos $w_h$ hasta cubrir una cierta cantidad de probabilidad cercana a 1, denominado un $\boldsymbol\epsilon$-\textbf{DP} [@muliere_approximating_1998].
...
```{=tex}
\begin{proposicion}[(Antoniak, 1974)]
...
Finalmente, al normalizar las medidas anteriores, obtenemos que
...
donde $\gamma$ es un parámetro de escala, esto es, $G_0 \equiv \text{Normal}(\mu | m, \gamma V)\cdot \text{IG}(V | s/2, S/2)$. Con esta especificación, obtenemos que:
...
\begin{enumerate}
  ...
  \end{itemize}
  \item $r_i = M \text{t}(y_i|s, m, \sqrt{S/s (1 + \gamma)})$, donde denotamos por $t(y_i|s, m, \sigma)$ la densidad de la distribución t-Student con $s$ grados de libertad, localización $m$ y escala $\sigma$, evaluada en $y_i$.
\end{enumerate}
```
```{=tex}
...
En este caso, el DPM parece ser más cercano a la densidad real (aunque claramente en la práctica no la conocemos). La implementación del algoritmo y la figura se encuentran en los códigos $\texttt{DpmNorm.jl}$ y $\texttt{examples/04\_DpmNormA1.jl}$, respectivamente.
\end{ejemplo}
```
...
Luego, el paso de simular de $\theta|\mathbf{y}$ podemos dividirlo en dos: simular primero de $\mathbf{c}|\mathbf{y}$ y luego de $\theta^*|\mathbf{c}, \mathbf{y}$. Para la segunda distribución, denotando por $\mathcal{C}_j = \{i\colon c_i = j\}$, tenemos que:

$$
\begin{aligned}
  p(\theta^*|\mathbf{c}, \mathbf{y}) &\propto p(\mathbf{y}|\theta)p(\theta^*|\mathbf{c}) \\
  &= \left[\prod_{j=1}^k \prod_{i \in \mathcal{C}_j} f_{\theta_j^*}(y_i) \right] \prod_{j=1}^k G_\eta(\theta_j^*) \\
  &= \prod_{j=1}^k \left[G_\eta(\theta_j^*) \prod_{i \in \mathcal{C}_j} f_{\theta_j^*}(y_i)\right]
\end{aligned}
$$

Donde la segunda distribución en el producto se obtiene al notar de ([-@eq-dp-marginal]) que un nuevo valor único siempre viene de $G_\eta$. Luego, tenemos que dados $\mathbf{c}$ e $\mathbf{y}$, cada uno de los valores únicos $\theta_j^*$, $j = 1,...,k$, son independientes, obteniendo finalmente que:

$$
p(\theta_j^*|\mathbf{c}, \mathbf{y}) \propto G_\eta(\theta_j^*)\prod_{i \in \mathcal{C}_j} f_{\theta_j^*}(y_i)
$${#eq-theta-a2}

Con esta información, podemos simular de la distribución deseada muestreando uno por uno $p(\theta_j^*|\mathbf{c}, \mathbf{y})$, $j = 1, ..., k$.
...
Mientras que en el segundo caso tenemos que:
...
de lo cual notamos una similitud con la función de densidad de una distribución Gamma, lo cual nos motiva a utilizar $M \sim \text{Gamma}(\alpha, \beta)$. Así, utilizando esta distribución a priori, tenemos que:

$$
\begin{aligned}
  p(M|\phi, k) &\propto M^{\alpha - 1} \exp\{-\beta M\} (M^{k} + M^{k-1}n)\exp\{M\log(\phi)\} \\
  &\propto M^{\alpha + k - 1}\exp\{-(\beta - \log(\phi))M\}  \\
  &\quad \quad \quad + nM^{\alpha + k - 2} \exp\{-(\beta - \log(\phi))M\} \\
  &\sim \pi_\phi \text{Gamma}(\alpha + k, \beta - \log(\phi)) + \\
  &\quad \quad \quad (1 - \pi_\phi) \text{Gamma}(\alpha + k - 1, \beta - \log(\phi))
\end{aligned}
$$
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item $\gamma_j^* = \gamma/(1 + n_j\gamma)$
    \item $s_j^* = s + n_j$
  ...
  \begin{enumerate}
    \item $\int f_\theta(y_i)dH_{-i, j} = t\left(y_i|s_{-i, j}^*, m_{-i, j}^*, \sqrt{\frac{S_{-i, j}^*}{s_{-i, j}^*}(1 + \gamma_{-i, j}^*)}\right)$
...
Como un ejemplo de lo anterior, Escobar y West (1995) aplican este modelo a datos que contienen velocidades de 82 galaxias. A través de diferentes fuentes, deciden utilizar como valores iniciales $w = 1$, $W = 100$, $\alpha = 2$, $\beta = 4$, $s = 4$ y $S = 2$, mientras que para $m$ utilizan una priori no informativa, por lo cual consideramos $a = 0$, $A = 1000$. Para el algoritmo, consideramos 1000 iteraciones con un periodo de quema de 500. En la Figura 12 se presenta tanto la densidad estimada, así como las distribuciones a priori y posteriori de $M$.

La implementación del algoritmo y la figura se encuentran en los códigos $\texttt{DpmNorm.jl}$ y $\texttt{examples/04\_DpmNormA2.jl}$, respectivamente.
\end{ejemplo}
```

::: {#fig-galaxies}
![](figures/DPM%20-%20galaxies%20example.png)

Ejemplo velocidades de galaxias. Vemos a la izquierda que la densidad parece considerar 5 componentes. En cuanto a la inferencia de $M$, vemos que pareciera estar centrada en 1, pero con una alta densidad en valores menores.
:::

\newpage
Antes de pasar al caso no conjugado presentaremos una generalización del modelo con verosimilitud Normal a observaciones multidimensionales, el cual utilizaremos en el capítulo de clustering. En este caso, utilizamos una verosimilitud Normal multivariada, que denotaremos por $\text{N}_p(\mu_i, \Sigma_i)$, donde $\mu_i$ es el vector p-dimensional de medias y $\Sigma_i$ la matriz de covarianzas. Para esta verosimilitud, una distribución conjugada es la Normal-Wishart-Inversa (NIW):
...
\mathbf{m}, \gamma &\sim \text{N}_p(\mathbf{a}, A)\text{IG}(w/2, W/2) \\
M &\sim \text{Gamma}(\alpha, \beta)
\end{aligned}
$$
...
\begin{itemize}
  \item Para $\eta|\theta^*$:
  \begin{enumerate}
    \item $\mathbf{m}|\gamma, \theta^* \sim \text{N}_p\left(\Lambda_m[A^{-1}\mathbf{a} + \gamma^{-1}\sum_{j=1}^k (\Sigma_j^*)^{-1}\mu_j^*]; \Lambda_m\right)$
    \item $\gamma|m, \theta^* \sim \text{IG}\left((w + kp)/2, (W + K)/2\right)$
  \end{enumerate}
  donde
  \begin{enumerate}
    \item $\Lambda_m = \left[A^{-1} + \gamma^{-1}\sum_{j=1}^k (\Sigma_j^*)^{-1}\right]^{-1}$
    \item $K = \sum_{j=1}^k (\mathbf{m} - \mu_j^*)^T (\Sigma_j^*)^{-1}(\mathbf{m} - \mu_j^*)$
  \end{enumerate}
  \item Para $p(\theta^*_j|\mathbf{c}, \mathbf{y})$ tenemos que
  $$
    p(\theta^*_j|\mathbf{c}, \mathbf{y}) \propto \text{NIW}(\mathbf{m}_j^*, \gamma_j^*, \Psi_j^*, \nu_j^*)
  $$
  donde
  \begin{enumerate}
    \item $\mathbf{m}_j^* = (\mathbf{m} + \gamma \sum_{i \in \mathcal{C}_j} \mathbf{y}_i)/(1 + \gamma n_j)$
    \item $\gamma_j^* = \gamma/(1 + n_j\gamma)$
    \item $\nu_j^* = \nu + n_j$
    \item $\Psi_j^* = \Psi + \frac{n_j}{1 + \gamma n_j}(\bar{\mathbf{y}}_{\mathcal{C}_j} - \mathbf{m})(\bar{\mathbf{y}}_{\mathcal{C}_j} - \mathbf{m})^T + \sum_{i \in \mathcal{C}_j} (\mathbf{y}_i - \bar{y}_{\mathcal{C}_j})(\mathbf{y}_i - \bar{y}_{\mathcal{C}_j})^T$
  \end{enumerate}
  \item La distribución $H_{-i, j}$ es idéntica a la anterior pero considerando $n_{-i, j}$ y $\mathcal{C}_{-i, j}$, obteniendo entonces que:
  \begin{enumerate}
    \item $\int f_\theta(y_i)dH_{-i, j} = t_p\left(\mathbf{y}_i\Big|\nu_{-i, j}^* - p + 1, \mathbf{m}_{-i, j}^*,\frac{1 + \gamma_{-i, j}}{\nu_{-i, j} - p + 1}\Psi_{-i, j}\right)$
    \item $\int f_\theta(y_i)dG_\eta = t_p\left(\mathbf{y}_i\Big|\nu - p + 1, \mathbf{m}, \frac{1 + \gamma}{\nu - p + 1}\Psi\right)$
  \end{enumerate}
  donde en este caso consideramos la densidad de una distribución $t$ p-variada.
\end{itemize}
```

### Caso no conjugado

En la sección anterior utilizamos modelos conjugados para así simular fácilmente de ([-@eq-theta-a2]) y calcular analíticamente las integrales en ([-@eq-cluster-a2]). Pese a esto, en ciertas situaciones nos interesará realizar elecciones arbitrarias tanto de $G_\eta$ como de $f_{\theta}$.

Al realizar esta elección es posible que perdamos las dos ventajas anteriores. Ahora, la simulación no es tan difícil de solucionar, considerando que podemos utilizar, por ejemplo, Metropolis-Hastings, HMC, o en realidad cualquier algoritmo que nos permita muestrear de una distribución arbitraria.

El problema principal entonces son las integrales. Siempre podemos recurrir a métodos numéricos para calcularlas, pero esto no suele ser muy eficiente en estos casos, aumentando demasiado el costo computacional y provocando una lenta convergencia a la distribución a posteriori deseada. En esta línea, hay dos algoritmos importantes que permiten trabajar con el caso no conjugado, los cuales introducen de dos formas diferentes valores de $\theta$ auxiliares que representan clusters vacíos aún no poblados. La diferencia es que el segundo algoritmo sí utiliza estos valores, mientras que el primero solo los incluye para la derivación de las distribuciones deseadas.

Ahora, es necesario realizar una pequeña modificación, ya que estos algoritmos sí consideran los valores de $\theta^*$ actuales para la actualización de $\mathbf{c}$, por lo que el algoritmo de Gibbs utiliza todas las condicionales completas, $p(\theta^*_j|\mathbf{c}, \mathbf{y})$ y $p(c_i|\mathbf{c}_{-i}, \theta^*, \mathbf{y})$. En particular, las distribuciones $p(\theta_j^*|\mathbf{c}, \mathbf{y})$ se mantienen iguales, pero las correspondientes a $c_i$ logran ser modificadas sin tener que calcular las integrales.

El primer algoritmo recibe el nombre de *no gaps*, propuesto por MacEachern y Müller [-@maceachern_estimating_1998]. Este algoritmo es bastante similar al anterior, pero se modifica en parte el caso que $c_i$ representa un cluster de una sola observación, esto es, $c_i \neq c_j$ para todo $j\neq i$. Estas modificaciones permiten reemplazar las integrales por evaluaciones en las verosimilitudes correspondientes.

El segundo algoritmo, conocido como el *Algoritmo 8* de Neal [-@neal_markov_2000], da cuenta de un problema de eficiencia del algoritmo no-gaps con respecto a la asignación de $c_i$ a un nuevo cluster. El algoritmo se deriva de una forma similar pero no idéntica, ya que en este caso sí utiliza las variables $\theta_A$ auxiliares. En particular, la idea es definir un modelo conjunto $q(\theta, \theta_A)$ tal que la distribución marginal sea $p(\theta)$ y la distribución de los $c_i$ no necesite integrar con respecto a alguna distribución de $\theta$. El número de estas componentes auxiliares lo denotaremos por $m$.

Al igual que antes, sea $k_{-i}$ el número de valores únicos de $\theta_{-i}$. Para actualizar $c_i$, la idea será extender el vector $\theta^*$ a un número $h = k_{-i} + m$ de componentes, para lo cual diferenciamos entre dos casos:

* Si $c_i = c_j$ para algún $j \neq i$, esto es, $c_i$ no representa un cluster con una única observación, entonces $\theta^*$ tendrá $k_{-i}$ componentes, por lo cual le agregamos $m$ valores de $G_\eta$

* Si $c_i$ sí representa un cluster de una sola observación, entonces $\theta^*$ tendrá $k_{-i} + 1$ componentes. En este caso, dejamos que $c_i = k_{-i} + 1$ e intercambiamos el $\theta^*$ correspondiente al final. Luego, solo debemos agregar $m - 1$ valores de $G_\eta$.

Luego, las probabilidades en ([-@eq-cluster-a2]) son reemplazadas por:

$$
p(c_i = j| c_{-i}, \mathbf{y}, \theta^*_1, ..., \theta^*_k, ..., \theta^*_h) \propto \begin{cases}
  n_{-i, j} f_{\theta^*_j}(y_i) & j = 1, ..., k_{-i} \\
  \frac{M}{m} f_{\theta^*_j}(y_i) & j = k_{-i} + 1, ..., h
\end{cases}
$$

Una vez que muestreamos de la distribución anterior, reordenamos $\mathbf{c}$ junto a $\theta^*$, dejando solo los valores que se encuentran asociados con algún cluster poblado.

Pese a que no es necesario, este algoritmo se encuentra implementado para el caso Normal en el código \texttt{DpmNorm.jl} y para el caso Normal Multivariado en el código \texttt{DpmMvNorm.jl}
...
Los algoritmos de clustering, que consisten en técnicas para agrupar observaciones similares según sus características, son ampliamente utilizados en el contexto no supervisado, con aplicaciones en diferentes áreas.

En el capítulo anterior, vimos cómo el utilizar un DPM nos permite definir un modelo probabilístico sobre clusters a través de $\mathbf{c}$. En este capítulo, nos enfocaremos en este producto del modelo, y de cómo podemos definir modelos de particiones aleatorias de forma general, así como la conexión entre medidas de probabilidades aleatorias discretas y clustering.

Denotemos por $[n] = \{1, ..., n\}$ las observaciones en un experimento. Una partición es un conjunto $\rho_n = \{\mathcal{C}_1, ..., \mathcal{C}_k\}$ de subconjuntos disjuntos y exhaustivos de $[n]$, esto es, $\mathcal{C}_i \cap \mathcal{C}_j = \emptyset$ para todo $i \neq j$ y $\cup_{j=1}^k \mathcal{C}_j = [n]$. Recordemos que una partición la podemos denotar de manera única con $\mathbf{c}$, donde $c_i = j$ si y sólo si $i \in \mathcal{C}_j$, siempre y cuando los valores en este vector no se salten la numeración.

### Definición mediante medidas discretas

Definimos entonces un modelo de partición aleatoria como una medida de probabilidad sobre el conjunto de todas las particiones posibles de $[n]$, que denotaremos por $\mathcal{P}_n$. Particularmente, este espacio es finito, donde el número de posibles particiones está dado por los números de Bell, que denotaremos por $B_n$.

¿Cómo definir entonces medidas de partición aleatoria? sin darnos cuenta, en el capítulo anterior se definió una de manera indirecta al utilizar un proceso de Dirichlet. De manera general, cualquier medida de distribución aleatoria discreta implica una medida de partición aleatoria, donde los grupos están dados por las igualdades de las observaciones de esta medida.

En particular, podemos obtener explícitamente la medida obtenida a partir de un Proceso de Dirichlet:
$$
\begin{aligned}
  p(\rho_n) &= p(\mathbf{c}) \\
  &= p(c_1) \prod_{i=2}^n p(c_i|c_1, ..., c_{i-1}) \\
  &= \frac{M^{k-1}\prod_{j=1}^k(n_j - 1)!}{(M + 1) \cdot\cdot\cdot (M + n - 1)}
\end{aligned}
$$

También, obtenemos explícitamente la medida obtenida a partir de un Proceso de Pitman-Yor:
$$
p(\rho_n) \propto M(M + b)\cdot\cdot\cdot(M + (k - 1)b)\prod_{j=1}^k (1 - b)(2 - b) \cdot\cdot\cdot ((n_j - 1) - b)
$$

Ahora, nos podemos preguntar acerca de por qué no trabajar directamente con $\mathcal{P}_n$ en vez de definir la medida de manera indirecta a través de una medida aleatoria discreta. Entre las razones se encuentra principalmente el teorema de representación de Kingman: decimos que una 
medida sobre $\mathcal{P}_n$ es **intercambiable** si $p(\rho_n) = f(n_1, ..., n_k)$ para alguna función $f$ simétrica y consistente, denominada **EPPF** (exchangeable partition probability function). El teorema nos dice que para toda medida $p(\rho)$ intercambiable existe una medida aleatoria discreta $G$ tal que el modelo sobre $\mathcal{P}_n$ es inducido por $G$. 

### Definición directa

Nos puede interesar también definir modelos de partición aleatoria directamente en el espacio $\mathcal{P}_n$, y no obtener medidas luego de marginalizar con respecto a una medida discreta. Una de estas opciones es la de **modelos de partición producto (PPM)** [@hartigan_partition_1990], las cuales toman la siguiente forma:

$$
p(\rho_n = \{\mathcal{C}_1, ..., \mathcal{C}_k\}) \propto \prod_{j=1}^k c(\mathcal{C}_j)
$$

donde las funciones $c$ reciben el nombre de **funciones de cohesión**. De hecho, si utilizamos $c(\mathcal{C}_j) = M(n_j - 1)!$, el modelo que nos queda es justamente el obtenido a partir de un Proceso de Dirichlet.

Otra opción es extender lo anterior para poder incluir covariables en las funciones de cohesión, lo cual se denomina un $\text{PPM}_x$ [@muller_product_2011]. En este caso, tenemos que:

$$
p(\rho_n = \{\mathcal{C}_1, ..., \mathcal{C}_k\}|X) \propto \prod_{j=1}^k c(\mathcal{C}_j)g(\mathbf{x}_j^*)
$$

donde $\mathbf{x}_j^* = \{\mathbf{x}_i \colon i \in \mathcal{C}_j\}$ y las funciones $g$ reciben el nombre de **funciones de similitud**.

## Inferencia a posteriori

Una vez que obtenemos la posteriori $p(\rho|\mathbf{y})$ nos podemos preguntar acerca de cómo poder resumir esta distribución en un valor puntual, considerando que no podemos calcular un "promedio" de clusters. Una opción es utilizar la moda a posteriori (MAP), pero está el problema que esta moda suele tener una probabilidad que no es significativamente alta, así como que los siguientes elementos con mayor probabilidad son muy diferentes entre sí.

De manera general, una de las ideas más utilizadas es elegir una partición que minimize alguna función de pérdida con respecto a la medida a posteriori de $\rho$. Si denotamos esta función por $L(\rho, \hat{\rho})$, donde $\rho$ corresponde al verdadero valor y $\hat{\rho}$ nuestra estimación, la idea es encontrar $\hat{\rho}_{\text{post}}$ tal que:

$$
\hat{\rho}_{\text{post}} = \operatorname*{argmin}_{\hat{\rho}} \int L(\rho, \hat{\rho})dP(\rho|\mathbf{y}) 
$$

Ahora, calcular esta integral de manera analítica puede ser complejo, por lo que generalmente hacemos uso de muestras de la posteriori $\rho|\mathbf{y}$ (equivalentemente, $\mathbf{c}|\mathbf{y}$). Esto es,

$$
\hat{\rho}_{\text{post}} = \operatorname*{argmin}_{\hat{\rho}} \left[\frac{1}{T}\sum_{t=1}^T L(\rho^{(t)}, \hat{\rho})\right]
$$

Algunos ejemplos de funciones de pérdida bastante utilizados en la práctica son:

* **Función de pérdida de Binder**: esta función de pérdida es bastante simple y lo que hace es asignar un costo $a > 0$ a que nuestra estimación no identifique dos observaciones en el mismo grupo cuando sí deberían estarlo, así como un costo $b>0$ en el caso que asigne a dos observaciones al mismo cluster cuando en realidad están separados. Esto es:
$$
L_{\text{B}}(\mathbf{c}, \hat{\mathbf{c}}; a, b) = \sum_{i < j} a \mathbb{1}\{c_i = c_j\}\mathbb{1}\{\hat{c}_i \neq \hat{c}_j\} + b \mathbb{1}\{c_i \neq c_j\}\mathbb{1}\{\hat{c}_i = \hat{c}_j\}
$$

* **Variación de la Información (VI)**: esta función de pérdida utiliza las entropías de $\rho$ y $\hat{\rho}$, así como la información mutua de ambos o, equivalentemente, las entropías individuales y la entropía conjunta. También existe una versión generalizada propuesta por Dahl [-@dahl_search_2022] que introduce pesos $a$ y $b$ en la misma línea que Binder.

El problema principal entonces será la búsqueda de esta partición. Es importante notar que el tamaño del espacio de particiones crece muy rápido con $n$, por lo que una búsqueda exhaustiva estará fuera de nuestros límites. Por ejemplo, para $n=10$ el número de particiones posibles es $B_{10} = 115975$, pero ya para $n=25$ tenemos $B_{25} = 4638590332229999353$. Si nos demorásemos 1 nanosegundo por cada partición, entonces tardaríamos 147 años en encontrar la partición óptima.

Dahl propone dos soluciones a lo anterior. La primera solución realiza la búsqueda y elección solo sobre el conjunto de muestras a posteriori con el método denominado *draws-based latent structure optimization (DLSO)* pero más conocido como el método de clustering de mínimos cuadrados [@do_model-based_2006]. Esta solución es bastante rápida, pero quedamos limitados solo a elegir entre configuraciones que hayan sido muestreadas. 

Una solución a lo anterior es el algoritmo SALSO [@dahl_search_2022] que sí realiza la búsqueda sobre el espacio completo. Este algoritmo se encuentra implementado en el paquete *salso* de R y acepta diferentes funciones de pérdida entre las cuales se encuentran las dos vistas anteriormente. Además, esta librería también permite utilizar el método DLSO. La Figura 12 presenta una aplicación del algoritmo SALSO, donde los datos fueron simulados a partir de 4 componentes, y utilizando tres funciones de pérdida diferentes. Notamos que en los tres casos tendemos a sobreestimar el número real de clusters.

::: {#fig-salso}
![](figures/SALSO.png)

Cluster estimado a partir del algoritmo SALSO considerando las funciones de pérdida Binder con $a = b$, VI y One Minus Adjusted Rand Index.
:::
...
Para apoyar en la lectura, en esta sección se presentarán los tres algoritmos MCMC vistos para el modelo DPM ([-@eq-dpm]) de manera resumida, paso por paso. Se utiliza la siguiente notación:

-   $\theta^*$: valores únicos de $\theta$

-   $k$: número de valores únicos de $\theta$

-   $\mathcal{C}_j$: índices de observaciones pertenecientes al cluster $j$, esto es, $\{i\colon c_i = j\}$

- $n_j$: número de observaciones en $\mathcal{C}_j$

- $k_{-i}$: número de valores únicos de $\theta$ sin considerar $\theta_i$

- $\mathcal{C}_{-i, j}$: índices de observaciones pertenecientes al cluster $j$ sin considerar la i-ésima observación

- $n_{-i, j}$: número de observaciones en $\mathcal{C}_{-i, j}$

-   $H_i(\theta) \propto f_{\theta}(y_i)G_\eta(\theta)$: posteriori obtenida de considerar una priori $G_\eta$ y verosimilitud $f_\theta$ con una única observación $y_i$.

-  $H_{-i, j}(\theta) \propto G_\eta(\theta)\prod_{l \in \mathcal{C}_{-i, j}} f_\theta(y_l)$: posteriori obtenida de considerar una priori $G_\eta$ y verosimilitud $f_\theta$ con todas las observaciones pertenecientes al cluster $j$, sin contar la i-ésima observación.

-   $q_{ij} \propto f_{\theta_j}(y_i)$

-   $r_i \propto M \int f_\theta(y_i)dG_\eta(\theta)$: priori predictiva del modelo con priori $G_\eta$ y verosimilitud $f_\theta$, evaluada en $y_i$.

### Algoritmo 1 - Estándar

El algoritmo consiste en:

-   Definir valores iniciales de $\theta$

-   Calcular los pesos $r_i$ y las distribuciones $H_i$ **antes** de partir con el algoritmo, ya que son constantes a través de las iteraciones.

-   Para $t = 1, …, T$:

    1.  Para $i = 1, …, n$:

        1.  Calcular los pesos $q_{ij}$, $j \neq i$

        2.  Muestrear de $p(\theta_i|\theta_{-i}, \mathbf{y}) \propto \sum_{j\neq i}q_{ij}\delta_{\theta_j} + r_iH_i$

    2.  Muestrear de $p(\theta_{n+1}|\theta) \propto \sum_{j=1}^k n_j \delta_{\theta_j^*} + MG_\eta$

### Algoritmo 2 - Sticky-Clusters e hiperparámetros

Ahora realizamos la separación de $\theta$ e incluimos los hiperparámetros $\eta$ y $M$. El algoritmo consiste en:

- Definir valores iniciales de $M, \eta, \mathbf{c}, \theta^*$
- Para $t = 1, ... T$:
    
    1.   Simular $\phi \sim \text{Beta}(M + 1, n)$ y calcular $\pi_\phi = (\alpha + k - 1)/[n(\beta - \log(\phi)) + \alpha + k - 1]$
    2.   Muestrear de  $$M|\phi, k \sim \pi_\phi\text{Gamma}(\alpha + k - 1, \beta - \log(\phi)) + (1 - \pi_\phi)\text{Gamma}(\alpha + k - 1, \beta - \log(\phi))$$
    3. Muestrear de $\eta|\theta^* \propto \pi_\eta(\eta) \prod_{j=1}^k G_\eta(\theta_j^*)$
    4. Para $i = 1, ..., n$:

        4.1 Muestrear de: $$p(c_i = j| c_{-i}, \mathbf{y}) \propto \begin{cases}
        n_{-i, j} \int f_\theta(y_i) dH_{-i, j}(\theta) & j = 1, ..., k_{-i} \\
        M\int f_\theta(y_i) dG_\eta(\theta) & j = k_{-i} + 1
        \end{cases}$$
        4.2 Reordenar los valores de $\mathbf{c}$ junto a $\theta^*$
    
    5. Para $j=1, ...k$:

        5.1 Muestrear de $p(\theta_j^*|\mathbf{c}, \mathbf{y}) \propto G_\eta(\theta_j^*)\prod_{i\in \mathcal{C}_j} f_{\theta_j^*}(y_i)$

    6. Muestrear de $p(\theta_{n+1}|\theta^*) \propto \sum_{j=1}^k n_j \delta_{\theta_j^*} + M G_\eta$
